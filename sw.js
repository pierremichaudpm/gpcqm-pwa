...
const CACHE_NAME = "gpcm-v4.3";
const RUNTIME_CACHE = "gpcqm-runtime-v4.3";
const IMAGE_CACHE = "gpcqm-images-v4.4";
const API_CACHE = "gpcqm-api-v4.2";
...
self.addEventListener("fetch", (event) => {
  ...
  if (
    ...
    // Note: navigator.userAgent is not available in service workers
        ...
        if (networkResponse && networkResponse.ok) {
          ...
          if (keys.length >= MOBILE_CACHE_CONFIG.maxImageCacheSize * 3) {
            ...
            const keysToDelete = keys.slice(
              0,
              keys.length - MOBILE_CACHE_CONFIG.maxImageCacheSize * 2,
            );
  ...
  for (const cacheName of cacheNames) {
    if (cacheName.includes("images")) {
      ...
      if (keys.length > MOBILE_CACHE_CONFIG.maxImageCacheSize) {
        ...
        const keysToDelete = keys.slice(
          0,
          keys.length - MOBILE_CACHE_CONFIG.maxImageCacheSize,
        );
        await Promise.all(keysToDelete.map((key) => cache.delete(key)));
      }
    }
